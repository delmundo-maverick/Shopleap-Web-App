...
<body class="min-h-screen bg-ice-blue text-charcoal">
    ...
    <main class="mx-auto max-w-7xl px-5 py-6 sm:px-8">
        ...
        <a href="{{ route('buyer.home') }}" class="mb-4 inline-flex items-center gap-1 text-xs text-charcoal/50 hover:text-primary">
        ...
        <div class="grid grid-cols-1 gap-8 rounded-lg bg-white p-6 shadow-sm shadow-charcoal/5 lg:grid-cols-2">
            ...
            <div>
                <div id="mainImageWrapper" class="aspect-square w-full overflow-hidden rounded-lg bg-ice-blue/60">
                    @if ($product->images->isNotEmpty())
                        <img id="mainImage" src="{{ $product->images->first()->url }}" alt="{{ $product->name }}" class="h-full w-full object-cover">
                    @else
            ...
            <!-- DETAILS -->
            <div>
                <span class="rounded bg-leaf-green/10 px-2 py-1 text-xs font-semibold text-leaf-green">New Arrival</span>

                <h1 class="mt-3 text-2xl font-bold text-charcoal">{{ $product->name }}</h1>

                <div class="mt-3 flex items-baseline gap-2">
                    <span class="text-3xl font-bold text-primary">₱{{ number_format($product->display_price, 2) }}</span>
                    @if ($product->was_price)
                        <span class="text-base text-charcoal/40 line-through">₱{{ number_format($product->was_price, 2) }}</span>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center rounded-lg border border-light-gray">
                        <button type="button" id="qtyMinus" class="px-3 py-2 text-charcoal/60 hover:text-primary">−</button>
                        ...
                        <button type="button" id="qtyPlus" class="px-3 py-2 text-charcoal/60 hover:text-primary">+</button>
                    </div>
                </div>
                ...
                <p class="whitespace-pre-line text-sm leading-relaxed text-charcoal/70">{{ $product->description ?: 'No description provided.' }}</p>
            </div>
    ...
    <!-- TOAST -->
    <div id="toast" class="fixed bottom-6 right-6 z-[80] hidden max-w-sm rounded-xl px-5 py-3.5 text-sm font-medium text-white shadow-xl transition"></div>
